...
export function generateThemeVariables(
    ...
    Object.entries(color.semantic).forEach(([variant, states]) => {
      Object.entries(states).forEach(([state, tokens]) => {
        Object.entries(tokens).forEach(([token, value]) => {
          declarations[`--color-${name}-${variant}-${state}-${token}`] = value;
        });
      });
    });
  });
  ...
  Object.entries(scheme.surfaces).forEach(([name, value]) => {
    declarations[`--surface-${toKebabCase(name)}`] = value;
  });

  const { text, ...themeTokens } = scheme.tokens;

  Object.entries(themeTokens).forEach(([name, value]) => {
    declarations[`--theme-${toKebabCase(name)}`] = value;
  });

  Object.entries(text).forEach(([name, value]) => {
    declarations[`--theme-text-${toKebabCase(name)}`] = value;
  });
